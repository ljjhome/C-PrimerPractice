...
public:
    ...
    std::string& operator[](std::size_t num){
        return elements[num]; 
    }
    const std::string& operator[](std::size_t num) const{
        return elements[num]; 
    }
    ~StrVec();
...
StrVec& StrVec::operator=(std::initializer_list<std::string> il){
    auto datanew = alloc.allocate(il.size());
    auto first = uninitialized_copy(il.begin(),il.end(),datanew);
    free();
    elements = datanew;
    first_free = first;
    cap = datanew + il.size();
